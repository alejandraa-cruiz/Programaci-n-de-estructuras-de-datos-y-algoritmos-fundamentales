...
uint Node<T>::leaves() const {
	...
	else{return this->left->leaves()+this->right->leaves();}
	return 0;
}
...
uint Node<T>::depth() const {
	...
	if(this->left==NULL && this-> right==NULL){return 1;}
		...
		uint depthL=this->left->depth();
		uint depthR=this->right->depth();
...
T Node<T>::ancestor(T val) const {
	//TO DO
	if (val == value){throw NoSuchElement();}

	if (left != NULL && left->value == val){return value;}

	else if (right != NULL && right->value == val){return value;}
...
public:
	...
	T ancestor(T) const;
	int height() const;
	void lev(Node<T> *, int, std::stringstream &) const;
};
...
std::string BST<T>::byLevel() const {
	...
	if (!empty()) {
		int h = height();
		for(int i = 0; i < h; i++){lev(root, i, aux);}
	}
...
template <class T>
void BST<T>::lev(Node<T> *root, int level, std::stringstream &aux) const {
	if(root == NULL){
		return;
	}
		...
		lev(root->left, level - 1, aux);
		lev(root->right, level - 1, aux);
	}
}
